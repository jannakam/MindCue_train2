...
// video.style.display = "none";  // Hide the video without removing it from the document flow
// video.play();
  if (isRecording) {
    toggleRecording();
  } else {
    ...
    // setInterval(() => {
    //   video.style.display = "none";  // Hide the video without removing it from the document flow
    //   video.style.opacity = "0";  // Make the video fully transparent
    // video.play();
    // }, 3000);
  }
});
...
const yolo_classes = [
  'accident', 'gun', 'soldier', 'spider', 'cockroach', 'wound'
];
